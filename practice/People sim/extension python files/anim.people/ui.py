...
class BehaviorControlUI:
    def __init__(self, ext_instance):
        ...
        self._CustomCommandPanel = ui_components.CustomCommandPanel(self, ext_instance)
        self._build()
    ...
    def shutdown(self):
        ...
        if self._CustomCommandPanel:
            self._CustomCommandPanel.shutdown()
            self._CustomCommandPanel = None
    ...
    def _build_content(self):
        with ui.ScrollingFrame(horizontal_scrollbar_policy=ui.ScrollBarPolicy.SCROLLBAR_ALWAYS_OFF):
            with ui.VStack():
                ...
                self._CustomCommandPanel.build()

...
from ...scripts.custom_command.populate_anim_graph import populate_anim_graph
...
class CharacterSetupWidget:
    ...
    def _build_content(self, title_stack, content_stack):
        with content_stack:
            with ui.HStack(spacing=50):
                ui.Spacer()
                self._character_load_button = ui.Button(
                    "Load Characters", width=200, height=10, alignment=ui.Alignment.H_CENTER
                )
                self._character_setup_button = ui.Button(
                    "Setup Characters", width=200, height=10, alignment=ui.Alignment.H_CENTER
                )
    ...
    def _load_characters_on_clicked(self):
        ...
        cmd_file_path = None
        # when stage do not have world prim.
        if world_prim and world_prim.IsValid() and world_prim.IsActive():
            if world_prim.HasAttribute(str(self.cmd_path_token)):
            ...
            if not cmd_file_path:
                carb.log_info("No people simulation commands provided. ")
        ...
        asyncio.ensure_future(self._init_characters(cmd_lines))

    async def _init_characters(self, cmd_lines):
        ...
        if people_asset_folder:
            ...
            root_path = await get_assets_root_path_async()
        ...
        if not self.stage.GetPrimAtPath("{}/{}".format(character_root_prim_path, self.default_biped_usd)):
            ...
            prim = prims.create_prim(
                "{}/{}".format(character_root_prim_path, self.default_biped_usd), "Xform", usd_path=biped_demo_usd
            )

...
class CommandWidget:
    def __init__(self, ext_ui_instance):
        ...
        self._stage_subscription = self._stage_update.create_stage_update_node(
            "CharacterControlPanel", on_attach_fn=self.on_attach
        )
            ...
            with ui.HStack(spacing=30):
                ...
                with ui.HStack(spacing=5):
                    ...
                    widget, model = create_setting_widget(
                        PeopleSettings.COMMAND_FILE_PATH,
                        SettingType.STRING,
                        range_from=0,
                        range_to=0,
                        speed=1,
    ...
    def _on_dir_pick(self, dialog: FilePickerDialog, filename: str, dirname: str):
        ...
        stage = omni.usd.get_context().get_stage()
        world_prim = stage.GetPrimAtPath("/World")
        # if world prim does not exist.
        if (world_prim is None) or (not world_prim.IsValid()):
            carb.log_warn("There is no /World prim inside stage. Create one as a place holder")
            # create an /world prim as a placeholder to store the command file information
